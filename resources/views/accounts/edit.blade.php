...
<div class="mb-6 flex justify-between items-center">
    <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Update your account details and balance.</p>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-2xl" x-data="{
    selectedIcon: '{{ $account->icon ?? 'fa-solid fa-wallet' }}',
    selectedColor: '{{ $account->color ?? '#3b82f6' }}',
    icons: [
    ...
    <form action="{{ route('accounts.update', $account) }}" method="POST" class="p-6 md:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" value="{{ old('name', $account->name) }}" required class="w-full rounded-lg border border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 p-2.5 outline-none placeholder-gray-400">
            </div>

            <!-- Currency (Read-only) -->
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Currency (Unchangeable)</label>
                <select disabled class="w-full rounded-lg border border-gray-200 bg-gray-50 text-gray-500 shadow-sm p-2.5 outline-none cursor-not-allowed">
            ...
            <div class="md:col-span-2">
                ...
                <input type="number" step="0.01" name="current_balance" value="{{ old('current_balance', $account->current_balance) }}" required class="w-full rounded-lg border border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 p-2.5 outline-none placeholder-gray-400">
            </div>
