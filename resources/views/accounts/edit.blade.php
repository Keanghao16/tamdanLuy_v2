...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div class="flex flex-col gap-4">
        <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-semibold text-gray-400">Edit Details</span>
        </div>
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Account</h1>
            <p class="text-sm text-gray-500 mt-0.5">Modify ledger metrics inside your localized tracking terminal channel context.</p>
        </div>
    </div>
    ...
    <form action="{{ route('accounts.destroy', $account) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this account?');">
        @csrf
        @method('DELETE')
        <button type="submit" class="bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2.5 rounded-xl text-sm font-bold shadow-2xs border border-red-100 transition flex items-center gap-2">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 w-full overflow-hidden" x-data="{
    selectedIcon: '{{ old('icon', $account->icon) ?? 'fa-solid fa-wallet' }}',
    selectedColor: '{{ old('color', $account->color) ?? '#3b82f6' }}',
    ...
    <form action="{{ route('accounts.update', $account) }}" method="POST" class="p-6 md:p-8 text-left">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $account->name) }}" required class="w-full text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary p-3 outline-none shadow-sm transition-all text-gray-700 placeholder-gray-400">
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="sm:col-span-1">
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Currency (Locked)</label>
                        <select disabled class="w-full text-sm border border-gray-200 bg-gray-50/70 text-gray-400 rounded-xl p-3 outline-none shadow-2xs cursor-not-allowed font-medium">
                            <option value="USD" {{ $account->currency === 'USD' ? 'selected' : '' }}>USD ($)</option>
                            <option value="KHR" {{ $account->currency === 'KHR' ? 'selected' : '' }}>KHR (៛)</option>
                        </select>
                    </div>

                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Current Balance</label>
                        <input type="number" step="0.01" name="current_balance" value="{{ old('current_balance', $account->current_balance) }}" required class="w-full text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary p-3 outline-none shadow-sm transition-all text-gray-700 placeholder-gray-400">
                    </div>
                </div>
            </div>
