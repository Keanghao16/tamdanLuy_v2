...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <span class="text-sm font-semibold text-gray-400">Edit Context</span>
        </div>
        <h1 class="text-2xl font-bold text-gray-900">Edit Budget</h1>
    </div>
    
    <form action="{{ route('budgets.destroy', $budget) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this budget?');" class="w-full sm:w-auto shrink-0">
        @csrf
        @method('DELETE')
        <button type="submit" class="w-full sm:w-auto bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2.5 rounded-xl font-bold text-sm shadow-sm border border-red-100/60 transition flex items-center justify-center gap-2">
            <i class="fas fa-trash-alt text-xs"></i> Delete Budget
        </button>
    </form>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 w-full" x-data="{
    ...
    <form action="{{ route('budgets.update', $budget) }}" method="POST" class="text-left">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="relative">
                        <select name="account_id" id="account_id" required class="w-full text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary p-3 outline-none shadow-sm transition-all text-gray-700 cursor-pointer">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" name="amount" id="amount" required min="0" step="0.01" value="{{ old('amount', $budget->amount) }}"
                               class="w-full text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary p-3 outline-none shadow-sm transition-all text-gray-700"
